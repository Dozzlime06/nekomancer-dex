...
// contracts/SwapAggregatorV4Upgradeable.sol
...
/// @title SwapAggregatorV4Upgradeable - UUPS Upgradeable Multi-DEX Swap
/// @notice 1% Fee (50/50 Split: Platform & Staking)
contract SwapAggregatorV4Upgradeable is 
    Initializable, 
    OwnableUpgradeable, 
    ...
    UUPSUpgradeable 
{
    ...
    event FeesDistributed(uint256 platform, uint256 staking);
    ...
    function _distributeFees(uint256 total) internal {
        ...
        emit FeesDistributed(half, total - half);
    }
    ...
    function swapMONForTokens(address tokenOut, uint256 minOut, uint256 deadline) external payable nonReentrant returns (uint256) {
        ...
        emit SwapExecuted(msg.sender, WMON, tokenOut, amountIn, amountOut, fee, dex);
        return amountOut;
    }

    function swapTokensForMON(address tokenIn, uint256 amountIn, uint256 minOut, uint256 deadline) external nonReentrant returns (uint256) {
        IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), amountIn);
        
        (address router, string memory dex) = _getBestRouter(tokenIn, WMON, amountIn);
        ...
        uint[] memory amounts = IUniswapV2Router(router).swapExactTokensForETH(amountIn, 0, path, address(this), deadline);
        uint256 rawOut = amounts[amounts.length - 1];
        ...
        emit SwapExecuted(msg.sender, tokenIn, WMON, amountIn, amountOut, fee, dex);
        return amountOut;
    }

    function swapTokensForTokens(address tokenIn, address tokenOut, uint256 amountIn, uint256 minOut, uint256 deadline) external payable nonReentrant returns (uint256) {
        ...
        uint[] memory amounts = IUniswapV2Router(router).swapExactTokensForTokens(amountIn, minOut, path, msg.sender, deadline);
        uint256 amountOut = amounts[amounts.length - 1];
        
        emit SwapExecuted(msg.sender, tokenIn, tokenOut, amountIn, amountOut, msg.value, dex);
        return amountOut;
    }
    ...
    function getVersion() external pure returns (string memory) { return "4.0.0"; }
